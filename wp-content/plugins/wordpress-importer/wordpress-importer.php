		...
		foreach ( (array) $_POST['imported_authors'] as $i => $old_login ) {
            ...
            if ( ! empty( $_POST['user_map'][$i] ) ) {
				...
				if ( ! empty($_POST['user_new'][$i]) ) {
				    $userexists = get_userdatabylogin($_POST['user_new'][$i]);
                    if ($userexists->ID>0) {
	...
	function process_posts() {
		foreach ( $this->posts as $post ) {
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
                ...
                // Begin get file url from post content
                $filetomedia = new filetomedia();
                $post_content = $filetomedia->change_imageurl_incontent($post,$author);
                // check if post meta value url
                if ( isset( $post['postmeta'] ) ) {
                    $post['postmeta'] = $filetomedia->change_fileurl_inmeta($post,$author);
				}
                // End get file url from post content
